...
story_list_file="${STORY_LIST_FILE:-tests/story-links.txt}"
...
usage() {
    cat <<'EOF'
Usage:
  bash tests/run-story-local.sh [story-url] [web-ext options...]
  bash tests/run-story-local.sh --all-stories [web-ext options...]

Options:
  --all-stories        Open all URLs from tests/story-links.txt
